...
function Home() {
  ...
  const { customPokemons, addCustomPokemon, deleteCustomPokemon, count } = useCustomPokemon();
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <div className="mt-10">
          <div className="flex items-center gap-4 mb-4">
            <h2 className="text-2xl font-bold text-gray-800">
              Mes Pokémon personnalisés ({count})
            </h2>
            <button
              onClick={() => setIsCustomModalOpen(true)}
              className="px-4 py-2 rounded-full text-white font-bold bg-indigo-600 hover:bg-indigo-700 transition-colors"
            >
              Créer un Pokémon
            </button>
          </div>
          ...
          {customPokemons.length === 0 ? (
            <p className="text-gray-400 italic">
              Aucun Pokémon personnalisé. Crée le tien !
            </p>
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 justify-items-center">
        ...
        <div className="mt-12 bg-gradient-to-r from-emerald-400 to-teal-500 rounded-xl p-6 text-white shadow-lg text-center">
          ...
          <div className="flex flex-wrap gap-4 justify-center">
            ...
            <div className="bg-white/20 p-3 rounded-lg">
              <h3 className="font-bold">Personnalisés</h3>
              <p>{count}</p>
            </div>
          </div>
        </div>
